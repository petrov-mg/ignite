...
| [03 — Cross-node propagation](03-cross-node-propagation.md) | `OperationContextDispatcher`, `OperationContextSnapshotMessage`, `DistributedAttributeRegistry`, the communication path, the TCP discovery path, the ZooKeeper discovery path |
...
├── OperationContextDispatcher.java        — distributed attribute registry + snapshot create/restore
├── OperationContextSnapshotMessage.java   — the wire format (built via its nested Builder)
