...
public class ThreadContext {
    ...
    /** */
    public static Scope withSnapshot(ThreadContextSnapshotRecord snapshotRec) {
        Scope scope = DefaultScope.create();

        for (ThreadContextAttribute<?> attr : ThreadContextAttributeRegistry.instance().attributes()) {
            Object val;

            if (!snapshotRec.isEmpty() && snapshotRec.attributeId() == attr.id()) {
                val = snapshotRec.value();

                snapshotRec = snapshotRec.next();
            }
            else
    ...
    /** */
    public static ThreadContextSnapshotRecord createSnapshot() {
