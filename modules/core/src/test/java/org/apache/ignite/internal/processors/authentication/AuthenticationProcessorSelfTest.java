...
public class AuthenticationProcessorSelfTest extends GridCommonAbstractTest {
    ...
    public void testRemoteNodeSecurityContext() throws Exception {
        try (ThreadContextScope ignored = grid(CLI_NODE).context().security().withContext(secCtxDflt)) {
        ...
        for (int i = 1; i < NODES_COUNT; i++) {
            ...
            try (ThreadContextScope ignored = security.withContext(subj.id())) {
    ...
    public static AutoCloseable withSecurityContextOnAllNodes(SecurityContext ctx) {
        List<ThreadContextScope> oldSecCtxs = new ArrayList<>();
        ...
        return new AutoCloseable() {
            @Override public void close() {
                oldSecCtxs.forEach(ThreadContextScope::close);
            }
        };
    }
}
